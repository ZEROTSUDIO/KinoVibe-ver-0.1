document.addEventListener('DOMContentLoaded', async () => {
  ...
  function selectTMDBMovie(movie) {
    ...
    if (previewEl && previewTitle) {
      ...
      if (previewGenres) previewGenres.textContent = (movie.genres || []).map(g => g.name).join(' · ');
      if (previewPoster) {
        const pUrl = TMDBService.getPosterUrl(movie.poster_path, 'w92');
        if (pUrl) {
  ...
  function updateScore() {
    ...
    scoreFinalEl.className = `score-big ${scoreGlow}`;
  }
